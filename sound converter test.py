...
app.overrideredirect(True)                  # no title bar, no border
ctk.set_appearance_mode("dark")
ctk.set_default_color_theme("green")

# Windows compositor: make this exact colour invisible → only widgets show
_KEY = "#010101"
app.configure(fg_color=_KEY)
app.wm_attributes("-transparentcolor", _KEY)

# +++++ Drag support — click-and-drag anywhere to move the window
_drag = [0, 0]

def _drag_start(e):
    _drag[0] = e.x_root - app.winfo_x()
    _drag[1] = e.y_root - app.winfo_y()

def _drag_move(e):
    app.geometry(f"+{e.x_root - _drag[0]}+{e.y_root - _drag[1]}")

app.bind("<ButtonPress-1>", _drag_start, add="+")
app.bind("<B1-Motion>",     _drag_move,  add="+")
app.bind("<Escape>",        lambda e: app.destroy())   # ESC to close
...
tabview = ctk.CTkTabview(app, width=480, height=480, corner_radius=14, fg_color=_KEY)
tabview.pack(padx=10, pady=10)
tabview.add("Converter")
tabview.add("YouTube Downloader")
tabview.tab("Converter").configure(fg_color=_KEY)
tabview.tab("YouTube Downloader").configure(fg_color=_KEY)
